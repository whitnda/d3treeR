...
#'
#' \code{d3tree4} is the primary function for creating interactive d3.js treemaps from
...
#'
#' @export
d3tree4Output <- function(outputId, width = '100%', height = '400px'){
  htmlwidgets::shinyWidgetOutput(outputId, 'd3tree4', width, height, package = 'd3treeR')
}

#' Widget render function for use in Shiny
#'
#' @export
renderd3tree4 <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) { expr <- substitute(expr) } # force quoted
  htmlwidgets::shinyRenderWidget(expr, d3tree4Output, env, quoted = TRUE)
}
